...
def apply_reimbursement_rules():
    ...
    if not active_rules:
        # Reset all rule-based reimbursements if no active rules
        for t in transactions:
            if t.get("reimbursement_source") == "rule":
    ...
    # Reset all rule-based reimbursements before reapplying
    for t in transactions:
    ...
    for rule in active_rules:
        ...
        for income_t in income_transactions:
            ...
            for deduction in deductions:
                ...
                total_deducted = round(total_deducted + deduct_amount, 2)
                ...
                expense_t["reimbursement_source"] = "rule"

            # Mark the income transaction with remaining amount
            ...
            if remaining_income <= 0:
                ...
                income_t["reimbursement_amount"] = 0.0
            else:
                income_t["reimbursement_status"] = "partial_rule"
                income_t["reimbursement_amount"] = remaining_income
...
def link_one_off_reimbursement(income_transaction_id, expense_transaction_id, reimbursement_amount):
    ...
    expense_t["reimbursement_source"] = "oneoff"

    # Set income transaction reimbursement amount
    ...
    if income_remaining <= 0:
        ...
        income_t["reimbursement_amount"] = income_remaining  # ← this was missing!
...
def unlink_reimbursement(transaction_id):
    ...
    if counterpart_id and counterpart_id != "multiple":
        counterpart = next(
...
def get_effective_amount(transaction):
    """Returns the effective display amount taking reimbursements into account."""
    ...
    if status in ("full_rule", "full_oneoff"):
        ...
        if reimb_amount is not None:
            try:
                reimb_amount = float(reimb_amount)
                # Preserve sign — income stays positive, expense stays negative
